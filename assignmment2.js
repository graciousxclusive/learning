...
class BankAccount {
    ...
    deposit(amount) {
        if (amount > 0) {
            ...
            const depositTransaction = new Transaction("deposit", amount);
            this.transactions.push(depositTransaction);
            return { message: `Deposit of $${amount} successful.`, success: true };
        } else {
            return { message: "Invalid deposit amount.", success: false };
        }
    }
    ...
    withdraw(amount) {
        if (amount > 0 && this.balance - amount >= 0 && this.withdrawalToday + amount <= this.dailyWithdrawalLimit) {
            ...
            const withdrawTransaction = new Transaction("withdrawal", amount);
            this.transactions.push(withdrawTransaction);
            return { message: `Withdrawal of $${amount} successful.`, success: true };
        } else if (amount <= 0) {
            return { message: "Invalid withdrawal amount.", success: false };
        } else if (this.balance - amount < 0) {
            return { message: "Insufficient funds.", success: false };
        } else if (this.withdrawalToday + amount > this.dailyWithdrawalLimit) {
            return { message: "Exceeded daily withdrawal limit.", success: false };
        }
    }
    ...
    transfer(amount, recipientAccount) {
        if (amount > 0 && this.balance - amount >= 0) {
            ...
            const transferTransaction = new Transaction("transfer", amount);
            this.transactions.push(transferTransaction);
            return { message: `Transfer of $${amount} to ${recipientAccount.accountHolder} successful.`, success: true };
        } else if (amount <= 0) {
            return { message: "Invalid transfer amount.", success: false };
        } else if (this.balance - amount < 0) {
            return { message: "Insufficient funds.", success: false };
        }
    }
    ...
    viewTransactions() {
        const transactionData = this.transactions.map(transaction => {
            return {
...
const account1 = new BankAccount(123456789, "Grace", "Umana", 1000);
const account2 = new BankAccount(987654321, "David", "Kelly", 1500);
